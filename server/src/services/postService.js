...
exports.trackInterest = async (postId, userId, action) => {
  ...
  await user.save();
  if (action === 'share') {
    const owner = await Post.findById(postId).select('author');
    if (owner) await notificationService.createNotification(owner.author, userId, 'repost', owner._id, 'post', 'reposted your post');
  }
  return { tracked: true };
};
...
exports.getStories = async (userId, query = {}) => {
  const since = new Date(Date.now() - 24 * 60 * 60 * 1000);
  const page = Math.max(parseInt(query.page, 10) || 1, 1);
  const limit = Math.min(Math.max(parseInt(query.limit, 10) || 60, 1), 100);
  const skip = (page - 1) * limit;
  const allowedAuthors = await getConnectedAuthorIds(userId);

  // Stories are paginated instead of capped at a tiny fixed number. This keeps
  // the UI smooth for very large networks while allowing an effectively
  // unlimited number of active stories over the 24-hour window.
  const [stories, total] = await Promise.all([
    ...
    Post.countDocuments({ author: { $in: allowedAuthors }, isStory: true, createdAt: { $gte: since } })
  ]);

  return { items: stories, page, limit, total, hasMore: skip + stories.length < total };
};
