...
exports.getStories = async (userId) => {
  ...
  // A stories rail should represent people, not every repost from the same
  // person. Keep the newest story per author so heavy posting/reposting can
  // never flood the client with duplicate story cards.
  const latestByAuthor = new Map();
  for (const story of stories) {
    const authorId = String(story.author?._id || story.author);
    if (!latestByAuthor.has(authorId)) latestByAuthor.set(authorId, story);
    if (latestByAuthor.size >= 24) break;
  }
