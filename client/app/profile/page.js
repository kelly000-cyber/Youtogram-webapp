...
const FACEBOOK_PROFILE_URL = 'https://www.facebook.com/profile.php?id=61593911235044';
...
function makeProfileForm(profile) {
  return {
    ...
    website: profile.website || FACEBOOK_PROFILE_URL, bio: profile.bio || '', avatar: profile.avatar || '', coverPhoto: profile.coverPhoto || ''
  };
}
...
export default function ProfilePage() {
  ...
  const handleImageChange = (event, field, maxBytes, setFileName, label) => {
    ...
    reader.onload = async () => {
      ...
      try {
        const response = await authService.updateProfile({ [field]: value });
        setProfile((current) => ({ ...current, ...response.data }));
        setForm((current) => ({ ...current, [field]: response.data?.[field] || value }));
        setMessage(`${label} saved successfully.`);
      } catch (saveError) {
        setError(saveError.message || `Unable to save ${label.toLowerCase()}.`);
      }
    };
