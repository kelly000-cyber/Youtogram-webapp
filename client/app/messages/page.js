...
export default function MessagesPage() {
  ...
  const handleSend = async (event) => {
    ...
    try {
      ...
      setError(sendError.message || 'Unable to send message. Check your connection and try again.');
    } finally {
      setSending(false);
    }
  };
        ...
        {loading ? (
          ...
          <>
            ...
            <form className="messagesComposer" onSubmit={handleSend}>
              ...
              <button type="submit" className="messagesSendButton" disabled={sending || (!draft.trim() && !attachment)}>{sending ? 'Sending…' : 'Send'}</button>
            </form>
          </>
        ) : (
