...
function AutoPlayReel({ video, styles }) {
  ...
  const handleVolumeChange = (event) => {
    ...
    // Unmute BEFORE changing volume. Native media controls can emit
    // volumechange synchronously; doing it in the opposite order used to
    // race React state and make raising volume immediately mute the video.
    media.muted = nextVolume === 0;
    media.volume = Math.min(Math.max(nextVolume / 100, 0), 1);
