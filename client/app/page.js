...
export default function HomePage() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      if (mode === 'login') {
        ...
        window.setTimeout(() => router.push('/feed'), 850);
      } else {
  ...
  return (
    <>
      {showLoginSplash ? (
        <div className="loginLogoSplash" role="status" aria-live="polite">
          <div className="loginLogoSplashRing">
            <img src="/youtogram.jpg" alt="Youtogram" />
          </div>
          <span>Welcome to Youtogram</span>
        </div>
      ) : null}
