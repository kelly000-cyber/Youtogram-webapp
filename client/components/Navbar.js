...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const refreshNotifications = () => notificationService.unreadCount().then((response) => setUnreadNotifications(response.data?.unreadCount || 0)).catch(() => {});
    refreshNotifications();
    const notificationTimer = window.setInterval(refreshNotifications, 15000);
    ...
    <nav className="navbar appTopbar">
      ...
      <div className="topbarCenter">
        {navItems.map((item) => (
          ...
          >
            {item.icon}
            {item.label === 'Alerts' && unreadNotifications > 0 ? <span className="navNotificationBadge">{unreadNotifications > 99 ? '99+' : unreadNotifications}</span> : null}
          </button>
        ))}
      </div>
